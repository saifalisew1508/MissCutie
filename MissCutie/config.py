...
class Config(object):
    ...
    DRAGONS = get_user_list("elevated_users.json", "1930139488")
    ##List of id's - (not usernames) for developers who will have the same perms as the owner
    DEV_USERS = get_user_list("elevated_users.json", "1930139488")
    ##List of id's (not usernames) for users which are allowed to gban, but can also be banned.
    DEMONS = get_user_list("elevated_users.json", "1930139488")
    # List of id's (not usernames) for users which WONT be banned/kicked by the bot.
    TIGERS = get_user_list("elevated_users.json", "1930139488")
    ...
    CASH_API_KEY = (
        "GT2SOXJ287A3TW05"  # Get your API key from https://www.alphavantage.co/support/#api-key
    )
    TIME_API_KEY = "BZ06IUOSX4GH"  # Get your API key from https://timezonedb.com/api
    WALL_API = (
