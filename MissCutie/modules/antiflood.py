...
from MissCutie import TIGERS, WOLVES, dispatcher
...
def check_flood(update, context) -> str:
    ...
    try:
        ...
        if getmode == 1:
            ...
            execstrings = "muted for ☞︎︎︎ {}".format(getvalue)
            tag = "TMUTE"
        send_message(
        ...
        msg.reply_text(
            "I can't restrict 🚫 users here, give me permission first until, i'll disable anti-flood lul OWNER."
        )
        sql.set_flood(chat.id, 0)
        return (
            "<b>{}:</b>"
            "\n#INFO"
            "\nI haven't enough permission to rstrict users so automatically disabled anti-flood".format(
                chat.title
            )
        )
...
def set_flood(update, context) -> str:
    ...
    if len(args) >= 1:
        ...
        if val in ["off", "no", "0"]:
            ...
            message.reply_text("Invalid agrument please use a number")
    else:
        message.reply_text(
            (
...
def flood(update, context):
    ...
    if conn:
        ...
        if update.effective_message.chat.type == "private":
            send_message(
                update.effective_message,
                "This comman is only for groups",
            )
            return
        chat_id = update.effective_chat.id
        chat_name = update.effective_message.chat.title
    ...
    if limit == 0:
        if conn:
            text = msg.reply_text(
                "I'm not forcing any flood control in  {}!".format(chat_name)
            )
        else:
