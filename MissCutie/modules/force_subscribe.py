...
from MissCutie import BOT_ID, DRAGONS, OWNER_ID, telethn
from MissCutie.events import register


# Constants
F_SUBSCRIBE_COMMAND = "/(fsub|Fsub|forcesubscribe|Forcesub|forcesub|Forcesubscribe)"
FORCESUBSCRIBE_ON = ["on", "yes", "enable"]
FORCESUBSCRIBE_OFF = ["off", "no", "disable"]
...
async def force_subscribe_new_message(e):
    ...
    if e.is_private or not e.from_id or e.sender_id in DRAGONS or e.sender_id == OWNER_ID:
        return
