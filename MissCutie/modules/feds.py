...
async def user_join_fed(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in DRAGONS:
        ...
        if (
            isinstance(get_owner, ChatMember)
            and user_id == get_owner.id
        ):
            await update.effective_message.reply_text(
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "total_fbanned_users.json"
                await update.effective_message.reply_document(
                    document=output,
                    filename="total_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "total_fbanned_users.csv"
                await update.effective_message.reply_document(
                    document=output,
                    filename="total_fbanned_users.csv",
...
async def fed_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            ...
            InlineKeyboardButton(text='Report Error', url='https://t.me/PublicSource_Chat')]]
        ),
    )
...
FED_HELP_BUTTON = CallbackQueryHandler(fed_help, pattern=r"fed_help_", block=False)
