...
from MissCutie import BOT_USERNAME, OWNER_ID, SUPPORT_CHAT, telethn
...
async def lego(event):
    ...
    try:
        ...
        fnt = glob.glob("./MissCutie/resources/fonts/*")
        ...
        await telethn.send_file(
            event.chat_id,
            file=fname,
            caption=f"logo generated By @{BOT_USERNAME}",
        )
        await pesan.delete()
        if os.path.exists(fname):
            os.remove(fname)
    except Exception:
        await event.reply(f"flood wait error, report this at @{SUPPORT_CHAT}")
