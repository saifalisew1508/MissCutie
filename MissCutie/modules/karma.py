...
regex_upvote = r"^((?i)\+|\+\+|\+1|thx|thanx|thanks|🖤|❣️|💝|💖|💕|❤|💘|cool|good|👍|baby|saif|thank you|gud|thankyou|love|pro)$"
...
async def upvote(_, message):
    ...
    if message.reply_to_message.from_user.id == OWNER_ID:
        await message.reply_text(
            "that's good but you know what, that person is my owner and everyone knows that he is a good man."
        )
        return
...
async def downvote(_, message):
    ...
    if message.reply_to_message.from_user.id == OWNER_ID:
        await message.reply_text(
            "wtf !, you don't agree with my owner. looks like you're not an good person."
        )
        return
    if message.reply_to_message.from_user.id == message.from_user.id:
        return
