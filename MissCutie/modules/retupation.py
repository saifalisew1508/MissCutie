...
async def increase(update: Update, context: CallbackContext):
...
async def decrease(update: Update, context: CallbackContext):
    msg = update.effective_message
    user1 = update.effective_user
    chat = update.effective_chat
    if not settings.chat_should_reputate(chat.id):
...
async def reputation(update: Update, context: CallbackContext):
    args = context.args
