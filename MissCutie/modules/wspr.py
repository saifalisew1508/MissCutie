...
async def wishper_ai(_, The: InlineQuery):
    ...
    if not query or len(query) > lengths or (query.startswith('@') and len(split) == 1):
        title = f"🔐 Write a whisper message"
        content = "**Send whisper messages through inline mode**\n\nUsage: `@MissCutieRobot [@username] text`"
        description = "Usage: @MissCutieRobot [@username] text"
        button = InlineKeyboardButton("More-Help", url="https://t.me/MissCutieRobot?start=learn")
        ...
        button = InlineKeyboardButton(f"🎯 show message", callback_data="show_whisper")
        ...
        button = InlineKeyboardButton(f"{emoji.LOCKED_WITH_KEY} show message", callback_data="show_whisper")
