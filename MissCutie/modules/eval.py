...
async def evaluate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.path.isfile("MissCutie/modules/helper_funcs/temp.txt"):
        os.remove("MissCutie/modules/helper_funcs/temp.txt")
...
async def do(func, bot, update):
    ...
    os.chdir(os.getcwd())
    with open(
        os.path.join(os.getcwd(), "MissCutie/modules/helper_funcs/temp.txt"), "w",
    ) as temp:
        temp.write(body)
